...
def get_institute_add_request_collection():
    """Get institute add request collection - ensures mongo is initialized"""
    return mongo.db.InstituteAddRequest
...
def get_institute_add_requests():
    try:
        ...
        result = list(get_institute_add_request_collection().aggregate(pipeline))
...
def get_institute_add_request_by_id(request_id):
    try:
        ...
        institute_add_request_data = get_institute_add_request_collection().find_one(
...
def create_institute_add_request():
    try:
        ...
        institute_address = data.get("institute_address")
        institute_map_link = data.get("institute_map_link", "")
        if not institute_name or not institute_address or not faculty_name:
            return (
                format_response(
                    False, "Institute name, faculty name, and address are required"
                ),
                400,
            )
        ...
        institute_add_request = InstituteAddRequestModel(
            ...
            institute_map_link=institute_map_link,
            created_by=user_id,
            updated_by=user_id,
        )
...
def toggle_processed_institute_add_request(request_id):
    try:
        ...
        institute_add_request = get_institute_add_request_collection().find_one(
            {"_id": ObjectId(request_id), "is_deleted": False}
        )
        ...
        get_institute_add_request_collection().update_one(
