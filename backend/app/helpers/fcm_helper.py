    ...
    def _initialize_firebase(self):
        ...
        try:
            ...
            if service_account_json:
                ...
                logger.info(
                    f"Using Firebase service account from file: {service_account_path}"
                )
            else:
                raise ValueError(
    ...
    def send_to_multiple_tokens(
        ...
        try:
            ...
            if response.failure_count > 0:
                for idx, resp in enumerate(response.responses):
                    if not resp.success:
                        ...
                        logger.warning(
                            f"Failed to send to token {failed_token}: {resp.exception}"
                        )
            ...
            return {
                ...
                "failed_tokens": [
                    {"token": token, "error": str(e)} for token in tokens
                ],
            }
